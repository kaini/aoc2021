...
static size_t hash_xy(const void *value) { return ((xy *)value)->x * 3109 + ((xy *)value)->y * 269; }

static bool equal_xy(const void *a, const void *b) {
    return ((xy *)a)->x == ((xy *)b)->x && ((xy *)a)->y == ((xy *)b)->y;
}

static bool is_diagonal(line line) { return line.x1 != line.x2 && line.y1 != line.y2; }
        ...
        if (!is_diagonal(*line)) {
            if (line->x1 == line->x2) {
                ...
                for (int y = y_start; y <= y_end; ++y) {
                    ...
                    if (!value)
                        value = map_put(&line_count, &key, &zero);
                    *value += 1;
                }
            } else {
